...
function hotfixScript(answers) {
    ...
    if (answers.upload) {
        ...
        script.push(`rsync -Pav -e "ssh -i $HOME/edelivery-key" odp_${repo.short.toLowerCase()}.$TAG.tar.bz2 ubuntu@edelivery.capiot.com:~/e-delivery/Releases/ODP/${answers.branch}/Hotfix/${repo.short}/${repo.short}-hotfix-${answers.hotfix}/`);
        script.push(`rsync -Pav -e "ssh -i $HOME/edelivery-key" yamls/${repo.short.toLowerCase()}.$TAG.yaml ubuntu@edelivery.capiot.com:~/e-delivery/Releases/ODP/${answers.branch}/Hotfix/${repo.short}/${repo.short}-hotfix-${answers.hotfix}/`);
        script.push(`echo "${chalk.green('***********************************')}"`);
        script.push(`echo "${chalk.green(`UPLOADING TO E-DELIVERY :: ${repo.name}`)}"`);
        script.push(`echo "${chalk.green('***********************************')}"`);
    }
